# Ideación y análisis de mercado – TFC Videojuego: Seaborn

## Índice

- [Pitch](#pitch)  
- [Género](#género)  
- [Personajes principales](#personajes-principales)  
- [Historia y ambientación](#historia-y-ambientación)  
- [Objetivo del juego](#objetivo-del-juego)  
- [Jugabilidad](#jugabilidad)  
- [Público objetivo](#público-objetivo)  
- [Diferenciación](#diferenciación)  
- [Tiempo de desarrollo estimado](#tiempo-de-desarrollo-estimado)  
- [Monetización](#monetización)  
- [Análisis de mercado](#análisis-de-mercado)  
- [Buyer Persona](#buyer-persona)  
- [Estrategia de comunicación](#estratregia-de-comunicación)  
- [Desarrollo](#desarrollo)  

## Pitch
_Separado de su familia desde bebé, Rafael crece en un centro aislado bajo estrictas reglas. Lo único que conoce del exterior son los libros de historia, que hablan de un mundo devastado por catástrofes naturales. Un evento inesperado le hace cuestionarse todo lo que conoce. Explora el centro, infíltrate en zonas prohibidas y descubre la verdad que oculta el lugar al que llamas hogar._

**Idea clave:** descubrir una mentira construida desde la infancia.

---

## Género
- Aventura narrativa  
- Sigilo  
- Exploración  
- Pixel art 2.5D  

---

## Personajes principales

### Rafael
Protagonista. Adolescente criado bajo control institucional. Evoluciona desde la obediencia hacia la rebelión y búsqueda de identidad.

### Pepa (La anciana)
Antagonista ambigua. Figura misteriosa, representa el conflicto moral del sistema. Personaje clave para conocer la verdad.

### Alejandro (El amigo)
Amigo cercano. Motor narrativo que impulsa la acción y el riesgo.

### NPCs
- Refuerzan la falsa normalidad  
- Algunos aportan pistas indirectas  

---

## Historia y ambientación
Ambientado en 2067 dentro del **C.O.A.C.**, una instalación que simula un mundo devastado por una catástrofe inexistente.

Los residentes han sido criados en aislamiento como parte del **Proyecto G.A.D.I.R.**, un experimento social sobre identidad restringida.

La ambientación mezcla:
- Rutina controlada (día)
- Sigilo y exploración (noche)
- Simulación tecnológica (hologramas, propaganda)

**Tema central:** manipulación de la verdad y construcción de la identidad.

---

## Objetivo del juego
Descubrir la verdad sobre el centro, el proyecto G.A.D.I.R y el mundo exterior.

El jugador deberá explorar, infiltrarse y evitar ser detectado recogiendo pistas en zonas prohibidas y finalmente intentar escapar del sistema.

---

## Jugabilidad

### Bucle principal
1. Día → interacción y narrativa  
2. Noche → sigilo y exploración  
3. Progreso → desbloqueo de zonas  

### Mecánicas clave
- Movimiento en 4 direcciones (2.5D)  
- Sistema de sigilo (guardias, cámaras y ruido)
- Inventario limitado  
- Sistema de documentos  
- Interacción contextual  

---

## Público objetivo
- Jugadores indie narrativos  
- Edad: 16–35  
- Usuarios de PC (Steam, Itch.io)  
- Interesados en storytelling y experiencias emocionales  

---

## Diferenciación
- Narrativa basada en manipulación institucional
- Sistema día/noche integrado en narrativa  
- Ambientación local (Cádiz) con simbología cultural  
- Antagonista moralmente ambiguo  
- Experiencia emocional guiada
- Estética minimalista con carga emocional  
- Ritmo pausado, sin combate

---

## Tiempo de desarrollo estimado
- MVP (Demo): 3 meses  
- Versión completa: 6 meses  

---

## Monetización
- Demo gratuita
- Juego completo (4-8€)

---

# ANÁLISIS DE MERCADO

## Segmentos de mercado

### Geográfico
- Europa (Especialmente España)
- Norteamérica  
- Japón

Implementación de traducciones al inglés y otros posibles idiomas.

### Demográficos
- Edad: 16–35  
- Jugadores indie narrativos  
- Consumidores de contenido narrativo  

### Psicográficos
- Interés por historias profundas  
- Atracción por distopías  
- Valoración de experiencias cortas pero intensas  

---

## Potencial de interés

El juego se sitúa dentro del mercado indie narrativo, caracterizado por:

- Crecimiento sostenido  
- Alta visibilidad en plataformas como Steam  
- Éxito de juegos con fuerte identidad narrativa  

### Factores que aumentan su potencial
- Temática universal (control, verdad, identidad)  
- Influencia de obras reconocibles (1984, Truman Show)  
- Estética pixel accesible  

**Conclusión:**  
Potencial medio-alto dentro de nicho indie, con posible capacidad de viralización.

---

## Ventaja competitiva narrativa
- Historia estructurada en actos claros  
- Personajes con arcos definidos  
- Temas profundos y actuales  
- Uso de narrativa ambiental (propaganda, arquitectura, rituales)

---

## Parámetros clave
Para encajar en el mercado objetivo, el juego debe:

- Tener un inicio impactante  
- Mantener tensión constante  
- Evitar mecánicas complejas
- Tener una estética reconocible
- Ofrecer narrativa clara pero intrigante  
- Incluir momentos memorables (giros narrativos)

---

## Buyer Persona

**Nombre:** Lucía / Alex  
- Edad: 22–30  
- Plataforma: PC  
- Intereses: juegos narrativos, arte, storytelling  

### Motivaciones
- Experimentar emociones  
- Descubrir juegos inides diferentes  
- Compartir recomendaciones  

### Frustraciones
- Juegos largos sin contenido relevante  
- Mecánicas complejas  
- Historias débiles

## Comportamiento
- Consume contenido en YouTube/Twitch y películas y series de buena narrativa. Le gusta leer.
- Comparte juegos recomendados  
- Valora estética y narrativa  

---

## Estratregia de comunicación

### Acciones de marketing
- Devlogs mostrando desarrollo  
- Clips de gameplay  
- Misterio en trailers (no revelar todo)  
- Demo en Steam  
- Participación en eventos indie  

---

### Estrategias de fidelización
- Construcción de comunidad  
- Transparencia en el desarrollo  
- Feedback de jugadores  
- Identidad de estudio  

---

## Desarrollo

### Producción

El desarrollo de *Seaborn* se plantea con un enfoque **iterativo y modular**, centrado en construir primero una base jugable sólida antes de escalar el contenido.

Se utilizará **Godot** como motor principal por su flexibilidad en 2D y su eficiencia para proyectos indie.

#### Enfoque de producción:
- Desarrollo por **salas o módulos independientes**, permitiendo testear cada zona de forma aislada  
- Implementación temprana de las **mecánicas core**:
  - Movimiento  
  - Sistema de sigilo (visión, ruido, luz)  
  - Interacción básica  
- Integración progresiva de narrativa (documentos, eventos, NPCs)  
- Uso de **placeholders** iniciales para priorizar jugabilidad sobre arte  

#### Fases de producción:
1. **Prototipo**
   - Movimiento y sigilo funcional  
   - Primer escenario jugable  
   - Testeo interno  

2. **Vertical Slice (Demo)**
   - Una experiencia completa representativa  
   - Introducción narrativa (Acto 1 + parte del Acto 2)  
   - Arte y audio cercanos al final  

3. **Producción completa**
   - Expansión de niveles  
   - Integración narrativa total  
   - Pulido de mecánicas  

4. **Pulido final**
   - Corrección de bugs  
   - Mejora de UX/UI  
   - Ajuste de ritmo narrativo  

---

### Publicación

La estrategia de publicación se centrará en maximizar visibilidad dentro del mercado indie sin depender de grandes inversiones.

#### Plataformas:
- **Steam (principal)**  
- **Itch.io (secundaria, comunidad indie)**  

#### Estrategia de lanzamiento:
- Publicación previa de una **demo gratuita**  
- Creación de página de Steam con antelación para generar **wishlist**  
- Lanzamiento acompañado de:
  - Trailer corto y directo  
  - Material visual atractivo (GIFs, clips)  

#### Marketing:
- Desarrollo abierto mediante **devlogs** en redes sociales  
- Publicación de clips centrados en:
  - Estética  
  - Sigilo  
  - Momentos narrativos  

- Participación en eventos indie online  

#### Objetivo principal:
Generar interés progresivo antes del lanzamiento en lugar de depender únicamente del día de salida.

---

### Gestión

La gestión del proyecto se enfocará en mantener un desarrollo **realista, controlado y sostenible**, evitando el aumento descontrolado del alcance.

#### Organización del trabajo:
- Uso de github como herramienta de planificación 
- División de tareas en bloques pequeños y alcanzables  
- Priorización de mecánicas esenciales sobre contenido extra  

#### Metodología:
- Enfoque **ágil** con iteraciones cortas  
- Revisión continua del progreso  
- Adaptación en función de problemas o feedback  
- Definición clara del **Producto Mínimo Viable (MVP)**  

#### Testeo:
- Pruebas internas constantes  
- Feedback de jugadores externos en fase demo  
- Ajustes en función de la experiencia del usuario  

#### Gestión de riesgos:
- Reducción de complejidad técnica  
- Reutilización de sistemas y assets  
- Planificación flexible ante imprevistos  
